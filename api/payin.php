...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($obj['search_text'])) {
    ...
    // SEARCH TEXT (party name / receipt no)
    if (!empty($search_text)) {
        $where .= " AND (JSON_EXTRACT(party_details, '$.party_name') LIKE ?
                    OR receipt_no LIKE ?)";
        $params[] = "%$search_text%";
        $params[] = "%$search_text%";
    }
    ...
    if (!empty($from_date) && !empty($to_date)) {
        $where .= " AND receipt_date BETWEEN ? AND ?";
        $params[] = $from_date;
        $params[] = $to_date;
    }
...
// Create Payin
    ...
    $Receiptno = BillNoCreation::create($params); // Ensure this function is defined

    // Update payin record with unique ID and receipt number
    $uniqueID = uniqueID("payin", $id); // Ensure this function is defined
    ...
    if ($updateResult['status'] !== 200) {
        error_log("Error updating payin: " . ($updateResult['msg'] ?? "Unknown error.")); // Log the error
        $output['status'] = 500;
